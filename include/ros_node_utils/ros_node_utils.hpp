...
inline bool paramRead(ros::NodeHandle *NodeHandle, std::string paramName,
                      Eigen::MatrixXd &variable, int row) {
  // std::cerr << "\033[0;32m" << paramName << "\033[0m" << std::endl;
  double *ptr;
  std::vector<double> buffer;
  if (NodeHandle->hasParam(paramName)) {
...
inline bool
paramRead(ros::NodeHandle *NodeHandle, std::string paramName,
          Eigen::SparseMatrix<double, Eigen::RowMajor> &variableSpace) {
  // std::cerr << "\033[0;32m" << paramName << "\033[0m" << std::endl;
  Eigen::MatrixXd variable = Eigen::MatrixXd();
  int prevSize = variable.rows();
  double *ptr;
  std::vector<double> buffer;
  if (NodeHandle->hasParam(paramName)) {
  ...
  variableSpace = variable.sparseView();
  return true;
...
inline bool paramRead(ros::NodeHandle *NodeHandle, std::string paramName,
  ...
  // std::cerr << "\033[0;31m" << variable.transpose() << "\033[0m" <<
  // std::endl;
}
